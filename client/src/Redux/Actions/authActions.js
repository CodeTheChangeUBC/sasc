...
export function signinCounsellor({ email, password }, history, callback) {
    return function(dispatch) {
        axios
            .post(`${ROOT_URL + BASE_URL}/tokens/counsellors`, {
                email,
                password
            })
...
export function signupCounsellor(
    ...
    return function(dispatch) {
        ...
        axios
            .post(`${ROOT_URL + BASE_URL}/counsellors`, data, header)
            .then(function(response) {
                dispatch({ type: AUTH_COUNSELLOR });
                localStorage.setItem("token", response.data.token);
...
export function signupUser(
    { ID, username, nickname, age, gender, phoneNumber, email, password },
    history,
    callback
) {
    return function(dispatch) {
        axios
            .post(`${ROOT_URL + BASE_URL}/users`, {
                ID,
                username,
                nickname,
                age,
